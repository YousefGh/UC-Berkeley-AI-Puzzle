    ...
    def chooseAction(self, gameState):
        ...
        for ghost in livingGhostPositionDistributions:
            for pos, prob in ghost.items():
                posProbVals.append((pos, prob))
        bestPos = max(posProbVals, key=lambda x: x[1])[0]  # best position according to the best probability (Greedy)

        # Best Action (from Best Position)
        actionDist = []
        for a in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, a)
            successorDist = self.distancer.getDistance(successorPosition, bestPos)
            actionDist.append((a, successorDist))
        bestAction = min(actionDist, key=lambda x: x[1])[0]
