    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for p in legalPos:
                ...
                probNoise = emissionModel[trueDistance]
                if probNoise > 0: allPossible[p] = self.beliefs[p] * probNoise
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in legalPos:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos)).items()
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for pos, val in beliefs.items():
                ...
                beliefs[pos] = val * emissionModel[dist]
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.particles = []
        legalPos = list(itertools.permutations(self.legalPositions, 2)) ###################################
        random.shuffle(legalPos)

        for particle in range(self.numParticles):
            self.particles.append(legalPos[particle % len(legalPos)])
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] is None:
                    ...
                    particleW *= emissionModels[i][dist]  # Noise
            w[particle] += particleW
        ...
        if w.totalCount() == 0:
            ...
            self.particles = []
            for weight in w.sortedKeys():
                s = w[weight] * self.numParticles
