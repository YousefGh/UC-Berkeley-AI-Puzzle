...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):                                       
        ...
        """YOUR CODE"""
        x = [1.0] + inActs
        # g = sum([self.sigmoid(x * weight) for x, weight in zip(x, self.weights)])
        g = self.sigmoid(self.getWeightedSum(x))
        return g
    ...
    def sigmoidDeriv(self, value):
        ...
        """
        """YOUR CODE"""
        grad = self.sigmoid(value) * (1 - self.sigmoid(value))
        return grad
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        """YOUR CODE"""
        x = [1.0] + inActs
        a = self.sigmoidDeriv(self.getWeightedSum(x))
        return a
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for idx, weight in enumerate(self.weights):
            grad = alpha * delta * inActs[idx]
            self.weights[idx] += grad

            totalModification += abs(grad)
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        """
        """YOUR CODE"""
        allPerceptrons = [inActs]
        for layer in self.layers:
            nextAct = [n.sigmoidActivation(inActs) for n in layer]
            allPerceptrons.append(nextAct)
            inActs = nextAct
    ...
    def backPropLearning(self, examples, alpha):
        ...
        for example in examples:#for each example
            ...
            allLayerOutput = self.feedForward(example[0]) # First example output
            ...
            for outputNum in xrange(len(example[1])):
                gPrime = self.outputLayer[outputNum].sigmoidActivationDeriv(allLayerOutput[-2]) #gradient
                error = example[1][outputNum] - lastLayerOutput[outputNum] # h - y
                delta = gPrime * error # adjustment strength (gradient * J)
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    delta = 0.0
                    for idx in xrange(len(nextLayer)):
                        delta += nextLayer[idx].weights[neuronNum + 1] * deltas[0][idx]
                    delta *= gPrime # weigh according to slope strength
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    totalModifications = weightChangeThreshold + 1
    for i in xrange(maxItr):
        iteration += 1
        if totalModifications < weightChangeThreshold:
            break
        J, totalModifications = nnet.backPropLearning(examplesTrain, alpha)
    ...
    for x, y in examplesTest:
        h = nnet.feedForward(x)[-1]
        predictions = []
        for i in xrange(len(h)):
            predictions.append(round(h[i]))
        if predictions == y:
            testCorrect += 1
        else:
            testError += 1

    testAccuracy= (1.0 * testCorrect) / (testCorrect + testError)  #num correct/num total
